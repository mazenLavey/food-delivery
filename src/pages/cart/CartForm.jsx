...
const CartForm = ({ getSumPrice, sendOrder }) => {
    ...
    return (
        <form className={CartFormCSS.form} onSubmit={handleSubmit}>
            ...
            <div className={CartFormCSS.total}>
                <p>Total: <strong>{priceFormatter(getSumPrice)}</strong></p>
                <button className={`${CartFormCSS.btn} ${isValid ? CartFormCSS.btnVaild : CartFormCSS.disabledBtn}`} type='submit' disabled={!isValid}>Submit Order</button>
            </div>
        </form>
